...
def get_params():
    ...
    p = [
        10**np.random.uniform(np.log10(1e-3), np.log10(0.1)), # eps (0) — сужаем до решаемого
        ...
        *np.random.uniform(-3, 3, size=3),  # q0, q1, q2 (10,11,12)
        ...
        np.random.uniform(-0.8, 0.8),  # j (17)
        np.random.uniform(-0.8, 0.8),  # k (18)
...
def run_generation(N=15000, filename="dataset_V4.csv", seed=42):
    np.random.seed(seed)
    ...
    for _ in tqdm(range(N), desc="Solving"):
        p = get_params()
        y_guess = np.vstack([
            np.linspace(p[1], p[2], len(x_nodes)),
            np.zeros(len(x_nodes))
        ])

        try:
            # Этап 1: грубый старт, высокая сходимость
            sol_coarse = solve_bvp(
                lambda x, y: ode_system(x, y, p),
                lambda ya, yb: bc(ya, yb, p),
                x_nodes, y_guess, tol=1e-3
            )
            if not sol_coarse.success:
                failed += 1

                continue

            # Этап 2: уточнение от грубого решения
            y_guess2 = sol_coarse.sol(x_nodes)
            sol_fine = solve_bvp(
                lambda x, y: ode_system(x, y, p),
                lambda ya, yb: bc(ya, yb, p),
                x_nodes, y_guess2, tol=1e-5
            )
            if not sol_fine.success:
                failed += 1
                continue

            # Аппроксимируем точное решение сплайном
            y_fine   = sol_fine.sol(x_nodes)[0]
    ...
    print(f"\nУспешно: {len(data)} / {N}  |  Провалилось: {failed} ({100*failed/N:.1f}%)")
    df = pd.DataFrame(data)
    df.to_csv(filename, index=False)
    print(f"Сохранено в {filename}")

if __name__ == "__main__":
    run_generation(N=50000, filename="dataset_V4.csv")